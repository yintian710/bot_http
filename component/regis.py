...
from tool.sql import select_wx_for_sql, update_wx_for_sql, insert_wx_for_sql, select_any_in_wx_for_sql
...
def is_wx_regis(openid):
    ...
    :return:
    """
    res = select_any_in_wx_for_sql({'openid': openid}, 'id')
    if not res:
...
def wx_regis(user_id, code, openid):
    if not code:
        return pa
    verify_code = select_wx_for_sql(user_id, 'code')[0]
    if not verify_code:
        return pa
    if verify_code != code:
        update_wx_for_sql(user_id, {'code': ''})
        return get_return('您输入的验证码不正确，请是重新获取', code=1)
    update_wx_for_sql(user_id, {"openid": openid, 'code': ''})
    return get_return(f'绑定成功，您的QQ号为：{user_id}')


@is_regis
def delete_wx_regis(user_id):
    update_wx_for_sql(user_id, {'code': '', 'openid': ''})
...
def get_verify_code(user_id):
    ...
    if not select_wx_for_sql(user_id, 'Id'):
        insert_wx_for_sql(user_id)
    update_wx_for_sql(user_id, {'code': verify_code})
